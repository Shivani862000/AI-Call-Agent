...
- **P09 admission is partially durable:** the pure decision contract remains green, migration `0021` now stores contact revisions/events and attempt reservations, and `/api/calls/initiate/:customerId` uses the transaction-backed reservation/idempotent response. Remaining manual, scheduler and diagnostic submitters plus concurrent/race/reconciliation evidence are open. [Execution plan](remediation-execution/P09-outbound-admission.md).
