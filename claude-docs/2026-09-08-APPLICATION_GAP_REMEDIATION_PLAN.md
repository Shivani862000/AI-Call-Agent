...
- [x] Define a stable pagination response `{ items, nextCursor, hasMore }`, default page size 50 and maximum 100. Whitelist filters/sort fields; use a stable `(created_at,id)` or explicitly chosen equivalent cursor. Update all callers atomically or supply an explicitly versioned compatibility adapter; do not silently change arrays to objects for one caller only.
