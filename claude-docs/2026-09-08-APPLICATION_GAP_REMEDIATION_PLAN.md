...
- **P06, database logs, dependencies, runtime packaging and provider metadata:** DB credential omission, compatible dependency updates, sanitized Node24 production-image verification and recursive provider metadata redaction are implemented and reviewed through `5eaa886` plus the current follow-up. The final isolated packaging/database/Node24/runtime run passed 380 checks; the focused log-privacy regression passes 1/1. Historical exposure assessment and live rotation decisions remain operational scope. [Evidence](GAP_REMEDIATION_EVIDENCE.md#p06--dependency-compatibility-and-audit).
...
- **P09 admission is partially durable:** the pure decision contract remains green, migration `0021` stores contact revisions/events and attempt reservations, all current outbound submitters use the transaction-backed reservation/idempotent response, and admission now locks the durable patient before budget checks. Unit **371/371** and DB **44/44** pass at current head; one focused concurrent two-queue-row race is covered. Provider reconciliation and the broader lifecycle/event/media race evidence remain open. [Execution plan](remediation-execution/P09-outbound-admission.md).
...
- **P14 reporting aggregates are partially implemented:** unrated positive calls no longer enter service recovery, recovery totals are computed before the bounded display queue, and owner alert cards/counts now use a full seven-day aggregate behind the bounded detail list. Unit **371/371** and DB **44/44** checks pass at current head. Other report/export aggregates and browser/date-boundary evidence remain open. [Execution plan](remediation-execution/P14-reporting-aggregates.md).
- **P15 campaign identity is partially implemented:** migrations `0025` and `0026` add/backfill durable queue and call campaign identity, preserve a historical label through queue deletion, and make reporting resolve spend by campaign ID after renames. Unit **371/371** and DB **44/44** pass through schema `0026`; import/attempt/UI propagation and production backfill review remain open. [Execution plan](remediation-execution/P15-campaign-identity.md).
- **P16 pagination has a bounded backend slice:** `GET /api/customers?page_size=&cursor=` and opt-in patient-list pagination now support bounded cursor traversal with 50-row defaults, 100-row maxima, filter preservation and malformed-input rejection while preserving legacy response shapes. Focused PostgreSQL regressions pass; browser consumption, import batching and performance measurement remain open. [Execution plan](remediation-execution/P16-customer-pagination.md).
...
| G5 — attribution and scale, schema `0026` | P15; P16 follows P07/P13/P14 and adds only measured index migrations. | Campaign identity and complete bounded lists/imports. |
...
`test:db`/`test:isolated` own provisioning, guarded migration, execution, and cleanup. `test:browser` fails explicitly until P17 implements its harness. Never substitute `TEST_DATABASE_URL=... npm run migrate`. Commit after zero-connect rejection and disposable execution evidence are recorded.
...
- [x] Update verified vulnerable dependencies while preserving Express4; pinned csv-parse7.0.2, Multer2.3.0, Nodemailer9.1.1 and qs6.16.0 override pass actual consumer checks, with zero audit findings at verification. Independent review closed at `f47f739`.
...
**Owner:** Backend/voice. **Gaps:** F16, recording portion of F08; R11. **Dependencies:** P05/P11; ships with `0023`.
...
- [x] Define stable bounded pagination responses for queue and patient lists, default page size 50 and maximum 100. Whitelist filters/sort fields; use stable cursor keys and preserve current response shapes as compatibility adapters.
...
- [ ] Convert Existing Patient/New Patient selection cards and search results to native keyboard-operable controls or a complete combobox/listbox pattern. The selection cards are now native; search/listbox semantics remain open.
- [x] Keep native radios in the accessibility tree/tab interaction; style visually without `display:none`. Preserve arrow-key group selection, labels and visible focus on the containing card.
- [x] Reuse the existing single dialog and its generated title ID. Focus an appropriate element on open/view change, contain Tab/Shift+Tab, make background inert, support Escape, and restore the actual opener or a stable fallback on close. Do not add a second dialog role to the backdrop.
...
- [x] Inventory inline script blocks, inline event handlers, `eval`/dynamic-function consumers, CDN scripts and DOM insertion points on every authenticated page and widget. The string-evaluation inventory is empty; the inline execution inventory remains for the next slice.
...
- [ ] Enforce `script-src` without `unsafe-eval` or `unsafe-inline`; `unsafe-eval` is now removed, while inline migration and enforcement remain open. Use self-hosted approved assets or a documented nonce/hash only where necessary. Keep style-policy decisions separate from script-policy closure.
