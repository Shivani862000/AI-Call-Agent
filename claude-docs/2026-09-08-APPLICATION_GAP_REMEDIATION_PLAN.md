...
- **P09 admission contract slice:** deterministic outbound request keys, payload-conflict detection, pause/capacity/daily/cooldown/contact decisions and provider-uncertainty classification are implemented in `services/outbound-admission.js`; five focused tests pass. Durable reservation schema, transaction integration across all submitters and concurrency/provider-uncertainty evidence remain open. [Execution plan](remediation-execution/P09-outbound-admission.md).
